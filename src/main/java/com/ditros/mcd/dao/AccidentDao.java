...
public interface AccidentDao extends JpaRepository<Accident, Long> {
    ...
    @Query("select a from Accident a " +
            "where a.crashDate LIKE %?1")
    List<Accident> getAccidentBymonthYear(String date);

    @Query(
        nativeQuery = true,
        value="select * from Accident a " +
    "where a.accident_severity_id=?2 and a.crash_date LIKE %?1")
List<Accident> getAccidentBymonthYearwithseverity(String date,Integer severity);
