...
public class CareService {
    ...
    public CareResp declareCare(CareReq careReq, String subject) throws Throwable {
        ...
        LocalDate dateNaiss;
        try{
        dateNaiss = LocalDate.parse(careReq.getDateNaiss(), formatter);
    }catch(Error e){
        ...
        if(!careReq.getContacts().isEmpty()&&careReq.getContacts()!=null){
                ...
                careReq.getContacts().stream().map(contactReq -> {
                    CareContact c = new CareContact(entity,
                            new Person(contactReq.getFirstName(),
                                    ...
                                    0.0,
                                    0.0,
                                    personGenderDao.getById(contactReq.getGender()==null? new Long(3):contactReq.getGender()),//=null? new Long(1):personGenderDao.getById(contactReq.getGender()),
                                    null,
                                    null,
                                    null),
...
public CareResp modifyCare(CareReq careReq, String subject) throws Throwable {
        System.out.println("careq---> "+careReq);
        Optional<User> hospitalEmployee = userDao.findByKeycloakId(subject);
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd");
